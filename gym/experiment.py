...
parser.add_argument('--model_type', type=str, default='dmamba',
                    help='Choose one of "dt", "dr", "dmamba-min" or "dmamba" (default: "dmamba")')
...
parser.add_argument('--speed_test', type=bool, default=False,
...
def main(variant):
    ...
    if dataset == 'medium-expert':
        dataset_path = data_dir_prefix + f'{env_name}-medium-expert-v2.pkl'
    ...
    if variant['speed_test'] == True:
        dataset_path = data_dir_prefix + 'lengthened_halfcheetah_medium_v2.pkl'
        with open(dataset_path, 'rb') as f:
            trajectories = pickle.load(f)
